...
const exam_completed = require('./exam_completed.js');
...
try{
    ...
    await db.query("select exam_completed.*,subjects.name as subject_name from exam_completed left join subjects on subjects.id = exam_completed.subject_id where student_id = "+student_id+" and exam_completed.subject_group_id = "+group_subject_id+" group by exam_completed.subject_id")
    .then(result=>{
        result.forEach(element=>{	
                subjectary.push(element['subject_id']);
                subjectnameary.push(element['subject_name']);
                chapterary.push(element['chapter_id'])
        
        })
    })
    chapterary = [...new Set(chapterary)];
...
let total_marks = 0;
    ...
    await db.query("select online_exam_question_answers.post_ans_status,exam_completed.subject_id,questions.branch,questions.branch_id,questions.question_type,questions.id,question_pattern.marks,questions.chapter_id from `exam_completed` left join online_exam_question_answers on \
    ...
    exam_completed.exam_type = 1 and exam_completed.student_id = "+student_id+" and exam_completed.subject_group_id = "+group_subject_id)
    .then(result=>{
        result.forEach(element=>{	
...
if(exam_type == 2){
    await db.query("select online_exam_question_answers.post_ans_status,exam_completed.subject_id,questions.branch,questions.branch_id,questions.question_type,questions.id,exam_completed.exam_set_counter from `exam_completed` left join online_exam_question_answers on \
    online_exam_question_answers.exam_unique_id = exam_completed.exam_unique_id left join questions on \
...
async function scholastic_chapter_wise_marks(req){
    try{
        ...
        let finalary = {};
        await db.query("select online_exam_question_answers.post_ans_status,exam_completed.branch_id as subject_id,questions.branch,questions.branch_id,questions.question_type,questions.id,question_pattern.marks,chapters.chapter_no from `exam_completed` left join online_exam_question_answers on online_exam_question_answers.exam_unique_id = exam_completed.exam_unique_id left join questions on questions.id = online_exam_question_answers.question_id left join question_pattern on question_pattern.short_code = questions.question_type  left join chapters on chapters.id = exam_completed.chapter_id where exam_completed.exam_category_id = 1 and exam_completed.exam_type = 1 and exam_completed.student_id = "+student_id+" and exam_completed.chapter_id != 0").then(result=>{
			result.forEach(element=>{	
				if(element['post_ans_status'] == 1){
					if(right_marks_subject_ary[element['subject_id']] == null){
						right_marks_subject_ary[element['subject_id']] = [];
