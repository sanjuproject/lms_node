...
router.post('/getexamscholasticdetails',adminMiddleware.validateToken, async function(req,res,next){
    try{
		...
		res.json(await exam_scholastic.getscholasticsubscribtion_details(req.body,purchased_packages['scholatic'],
...
router.post('/getexamcompetitivedetails',adminMiddleware.validateToken,  async function(req,res,next){
    try{
		let purchased_packages = await purchased_subscribtions.get_purchased_subscription_details(req.body);
		req.body.id = req.user.id;
		res.json(await exam_competitive.getcompetitivesubscribtion_details(req.body,purchased_packages['competive'],
					...
					//updated_subscribtion_details[0]['cart_amount'] = req.body.amount_paid;
						...
						.then(async response=>{
							if(response.status == 200)
							{
								...
								.then(async result=>{
									//let reqest_data = {email:result.email,subject:configdata.studentenrolmentmail.subject,body:configdata.studentenrolmentmail.body}
									//helper.sendmail(reqest_data);
									
									//let reqest_data3 = {email:result.email,subject:configdata.studentenrolmentmail_2.subject,body:configdata.studentenrolmentmail.body}
									//helper.sendmail(reqest_data3);
...
router.post('/get_subscription_dashboard_details',adminMiddleware.validateToken,  async function(req,res,next){
    ...
    let competive_details = [];
		...
		res.status(200).send({status:200,msg:"Student Subscription status",total_scholastic_master:total_scholastic_master.length,
...
router.post('/e_library_subscription_list_competetive', adminMiddleware.validateToken, async function(req,res,next){
	try{
		req.body.id = req.user.id;
		let purchased_packages = await purchased_subscribtions.get_purchased_subscription_details(req.body);
		res.json(await e_library_subscription_master.list_competetive(req.body,purchased_packages['elibrary_com'],
...
router.post('/e_library_subscription_list_scolastic',adminMiddleware.validateToken,  async function(req,res,next){
	try{
		...
		res.json(await e_library_subscription_master.list_scolastic(req.body,purchased_packages));
	}
	catch(err){
		console.error(`Error while getting programming languages `, err.message);
    	next(err);
...
router.post('/e_library_purchased_subject_list_competitive',adminMiddleware.validateToken, async function(req, res, next){
	try{
		res.json(await purchased_subscribtions.list_competitive_library(req.body));
	}
	catch(err){
		console.error(`Error while getting programming languages `, err.message);
    	next(err);
...
router.post('/getsubscriptiondetails',adminMiddleware.validateToken, async function(req, res, next){
	try{
		...
		.then(result=>{
			response = {status:200,"msg":"Congratulations! Your subscription purchase has been processed successfully.",email:result[0].email,exam_unique_id:result[0].exam_unique_id,is_subscribe:result[0].is_subscribe,
		...
		.then(result=>{
			if(result.length > 0){
				result.forEach(element=>{
					...
					purchased_packages.push({category_id:1,category:element.category,subject_name:element.subject_name,subject_image:element.subject_image,
		...
		.then(result=>{
			if(result.length > 0){
				result.forEach(element=>{
					...
					purchased_packages.push({category_id:2,category:element.category,subject_name:"Competitive",subject_image:element.subject_image,
						subject_color_code:element.subject_color_code,no_set:element.no_set,mock_count:mock_count,module_count:module_count,
						case_study:case_study,elibrary:elibrary})
				})
			}
