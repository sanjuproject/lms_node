...
//////////////////////////////// PAGE No 1 BOX 1 ///////////////////////////////////////////////
//Get PSC Scholactic Overall Average
...
///////////////////// PAGE NO 3 BOX ALL ////////////////////////////////////////
//Scholastic subject wise analysis on set
	...
	for (const key in details_chapter_exam_ary_interm) {
		...
		for (const key2 in details_chapter_exam_ary_interm[key])
		{
			...
			if(total_question > 14 && exam_type == 1){
				total_exam_marks = total_exam_marks_ary.reduce((acc, value) => acc + value, 0);
			}
...
//chapter wise analysis on set Scholastic
...
////////////////////////////////////////////////// PAHE 4 BOX ALL //////////////////////////////////
//chapterwise analysis on set Scholastic Case Study
...
//Competititve Subject Average Score vs Market Trend
...
//Competitve Set Wise SAT Score subject wise
...
//Competitve Set Wise Score Comparison 
...
//Competitve Subject wise Comparasion
...
router.post('/compare_scholastic_competitive' ,adminMiddleware.validateToken , async function(req,res,next){
	...
	if(exam_type != undefined)
	{
	await db.query("select subjects.* from `subjects` left join  exam_type on  exam_type.id = subjects.exam_type_id where `subjects`.`exam_category_id` = 1 and `subjects`.`is_deleted` = 0 and `subjects`.`status` = 1 and `subjects`.`group_exist` = 2 and `subjects`.`board_id` = "+board_id)
	.then(result=>{
...
//E-library Session Time Spent API
...
//E-library Most Visited Subjects
...
//E-library Most Searched Keyword
...
//////////////////////////// Page 3 CARD NO 3 where do you stand_competitive ///////////////////////////////
//wheredoyoustand_competitive
...
//Overall Competitive(NTSE &/or NSTSE) Performance Score(Avg.)
...
router.post('/scholastic_getsubjectwise_chapters_table_data',adminMiddleware.validateToken , async function(req,res,next){
			...
			if(result.length > 0)
			{
				let exam_set_counter_ary = [];
					...
					result.forEach(element=>{
						...
						if(details_chapter_exam_ary_interm[element.chapter_no] == null){
							details_chapter_exam_ary_interm[element.chapter_no] = [];
							//details_chapter_exam_ary[element.exam_set_counter] = [];
							//details_chapter_exam_ary_interm_incorrent_ans[element.exam_set_counter] = [];
						}

						details_chapter_exam_ary_interm[element.chapter_no].push(element); 
	...
	for (const key in details_chapter_exam_ary_interm) {
		...
		if(details_chapter_exam_ary[details_chapter_exam_ary_interm[key][key2].chapter_no] == null)
		{
			details_chapter_exam_ary[details_chapter_exam_ary_interm[key][key2].chapter_no] = {};
		}
		if(details_chapter_exam_ary[details_chapter_exam_ary_interm[key][key2].chapter_no]["Test"+details_chapter_exam_ary_interm[key][key2].exam_set_counter] == null)
		{
			details_chapter_exam_ary[details_chapter_exam_ary_interm[key][key2].chapter_no]["Test"+details_chapter_exam_ary_interm[key][key2].exam_set_counter] = {};
		}
	...
	for(const key in details_chapter_exam_ary)
	{
		for(const key2 in details_chapter_exam_ary[key])
		{
			...
			let total_marks = 0;
			details_chapter_exam_ary_interm[key].forEach(element=>{
