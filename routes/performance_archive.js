...
const exam_completed_archive = require('../services/exam_completed.js');
const helper = require('../helper.js');
const { config } = require('dotenv');
const configdata  = require('../config.js');
const db = require('../services/db.js');
const chapters = require('../services/chapters.js');
const performance_service = require('../services/performance_service_archive.js');
...
router.post('/getscholasticstrengthanalysis',adminMiddleware.validateToken , async function(req,res,next){
    try{
		...
		let group_subjects = "";
		await db.query("select exam_completed_archive.*,subjects.name as subject_name from exam_completed_archive left join subjects on subjects.id = exam_completed_archive.subject_id where exam_completed_archive.previous_class = "+class_no+" and exam_completed_archive.student_id = "+student_id+" and exam_completed_archive.subject_group_id = "+group_subject_id+" group by exam_completed_archive.subject_id")
		.then(result=>{
			result.forEach(element=>{	
		...
		await db.query("select exam_completed_archive.*,subjects.name as subject_name from exam_completed_archive left join subjects on subjects.id = exam_completed_archive.subject_id where exam_completed_archive.previous_class = "+class_no+" and exam_type = 2 and student_id = "+student_id+" and exam_completed_archive.subject_group_id = "+group_subject_id+" group by exam_completed_archive.subject_id")
		.then(result=>{
			result.forEach(element=>{	
		...
		await db.query("select exam_completed_archive.*,subjects.name as subject_name from exam_completed_archive left join subjects on subjects.id = exam_completed_archive.subject_id where exam_completed_archive.previous_class = "+class_no+" and exam_type = 3 and student_id = "+student_id+" and exam_completed_archive.subject_group_id = "+group_subject_id+" group by exam_completed_archive.subject_id")
		.then(result=>{
			result.forEach(element=>{	
...
let total_marks = 0;
		...
		await db.query("select online_exam_question_answers.post_ans_status,exam_completed_archive.subject_id,questions.branch,questions.branch_id,questions.question_type,questions.id,question_pattern.marks from `exam_completed_archive` left join online_exam_question_answers on online_exam_question_answers.exam_unique_id = exam_completed_archive.exam_unique_id left join questions on questions.id = online_exam_question_answers.question_id left join question_pattern on question_pattern.short_code = questions.question_type where exam_completed_archive.previous_class = "+class_no+" and `exam_category_id` = 1 and exam_completed_archive.exam_type = 1 and exam_completed_archive.student_id = "+student_id+" and exam_completed_archive.subject_group_id = "+group_subject_id+"")
		.then(result=>{
			result.forEach(element=>{	
...
let total_module_subject = 0;
		...
		questions.id = online_exam_question_answers.question_id where exam_completed_archive.previous_class = "+class_no+" and `exam_category_id` = 1 and \
...
/////////////////////// SET EXAM Performance ///////////////////////////////
		...
		await db.query("select online_exam_question_answers.post_ans_status,exam_completed_archive.subject_id,questions.branch,questions.branch_id,questions.question_type,questions.id from `exam_completed_archive` left join online_exam_question_answers on online_exam_question_answers.exam_unique_id = exam_completed_archive.exam_unique_id left join questions on questions.id = online_exam_question_answers.question_id where exam_completed_archive.previous_class = "+class_no+" and `exam_category_id` = 1 and exam_completed_archive.exam_type = 1 and exam_completed_archive.student_id = "+student_id+" and exam_completed_archive.subject_group_id = "+subject_group_id)
		.then(result=>{
			result.forEach(element=>{	
				if(element['post_ans_status'] == 1){
					if(right_marks_subject_ary[element['subject_id']] == null){
...
let module_total_marks_subject_ary = {};
		...
		await db.query("select online_exam_question_answers.post_ans_status,exam_completed_archive.subject_id,questions.branch,questions.branch_id,questions.question_type,questions.id from `exam_completed_archive` left join online_exam_question_answers on online_exam_question_answers.exam_unique_id = exam_completed_archive.exam_unique_id left join questions on questions.id = online_exam_question_answers.question_id where exam_completed_archive.previous_class = "+class_no+" and `exam_category_id` = 1 and exam_completed_archive.exam_type = 2 and exam_completed_archive.student_id = "+student_id+" and exam_completed_archive.subject_group_id = "+subject_group_id)
		.then(result=>{
			result.forEach(element=>{	
				if(element['post_ans_status'] == 1){
					if(module_right_marks_subject_ary[element['subject_id']] == null){
...
let mock_total_marks_subject_ary = {};
		...
		await db.query("select online_exam_question_answers.post_ans_status,exam_completed_archive.subject_id,questions.branch,questions.branch_id,questions.question_type,questions.id from `exam_completed_archive` left join online_exam_question_answers on online_exam_question_answers.exam_unique_id = exam_completed_archive.exam_unique_id left join questions on questions.id = online_exam_question_answers.question_id where exam_completed_archive.previous_class = "+class_no+" and `exam_category_id` = 1 and exam_completed_archive.exam_type = 3 and exam_completed_archive.student_id = "+student_id+" and exam_completed_archive.subject_group_id = "+subject_group_id)
		.then(result=>{
			result.forEach(element=>{	
				if(element['post_ans_status'] == 1){
					if(mock_right_marks_subject_ary[element['subject_id']] == null){
...
router.post('/getscholasticaverageperformance_set_module_mock',adminMiddleware.validateToken , async function(req,res,next){
    try{
		...
		await db.query("select * from `exam_completed_archive` left join `online_exam_question_answers` on `online_exam_question_answers`.`exam_unique_id` = `exam_completed_archive`.`exam_unique_id`\
		 where exam_completed_archive.previous_class = "+class_no+" and `exam_completed_archive`.`exam_type` = 2")
		.then(result=>{
			result.forEach(element=>{
...
await db.query("select online_exam_question_answers.*,questions.branch,subjects.id as subject_id from online_exam_question_answers\
 left join exam_completed_archive on exam_completed_archive.exam_unique_id = online_exam_question_answers.exam_unique_id left join\
 questions on questions.question_no = online_exam_question_answers.question_no left join\
  branches on branches.branch_code = questions.branch left join subjects on subjects.id = branches.subject_id\
   where exam_completed_archive.previous_class = "+class_no+" and `online_exam_question_answers`.`student_id` = "+req.body.student_id+" and `exam_completed_archive`.`exam_type` = 2")
...
await db.query("select online_exam_question_answers.*,questions.branch,subjects.id as subject_id from online_exam_question_answers\
 ...
 questions on questions.question_no = online_exam_question_answers.question_no left join\
  branches on branches.branch_code = questions.branch left join subjects on subjects.id = branches.subject_id\
   where exam_completed_archive.previous_class = "+class_no+" and `online_exam_question_answers`.`student_id` = "+req.body.student_id+" and `exam_completed_archive`.`exam_type` = 3")
...
router.post('/scholastic_getsubjectwise_chapters',adminMiddleware.validateToken , async function(req,res,next){
    try{
		...
		let performance_right_score_chapterwise = [];
		let performance_full_score_chapterwise = [];
		let total_question_css_group = 5;
	...
	{	
		...
		.then(result=>{
			...
			//console.log(result);
			performance_right_score_chapterwise = result.chapterary_right_marks;
			performance_full_score_chapterwise = result.chapterary_total_marks;
		})
...
let chapter_ary = {};
		await db.query("select exam_completed_archive.*,subjects.name as subject_name,chapters.chapter_no,online_exam_question_answers.post_ans_status,online_exam_question_answers.post_ans,online_exam_question_answers.question_id,questions.question_type from `exam_completed_archive` left join `online_exam_question_answers` on `exam_completed_archive`.`exam_unique_id` = `online_exam_question_answers`.`exam_unique_id` left join subjects on subjects.id = exam_completed_archive.subject_id left join chapters on chapters.id = exam_completed_archive.chapter_id left join questions on questions.id = online_exam_question_answers.question_id where exam_completed_archive.previous_class = "+class_no+" and `exam_completed_archive`.`exam_type` = "+exam_type+" and `exam_completed_archive`.`student_id` = "+student_id+" and `exam_completed_archive`.`chapter_id` != 0 and chapters.standard = "+standard+" and exam_completed_archive.subject_group_id = "+group_subject_id+" GROUP by exam_completed_archive.chapter_id order by chapters.chapter_no asc")
		.then(result=>{
			if(result.length > 0)
			{
				result.forEach(element=>{
		...
		await db.query("select exam_completed_archive.*,subjects.name as subject_name,chapters.chapter_no,online_exam_question_answers.post_ans_status,online_exam_question_answers.post_ans,online_exam_question_answers.question_id,questions.question_type from `exam_completed_archive` left join `online_exam_question_answers` on `exam_completed_archive`.`exam_unique_id` = `online_exam_question_answers`.`exam_unique_id` left join subjects on subjects.id = exam_completed_archive.subject_id left join chapters on chapters.id = exam_completed_archive.chapter_id left join questions on questions.id = online_exam_question_answers.question_id where exam_completed_archive.previous_class = "+class_no+" and `exam_completed_archive`.`exam_type` = "+exam_type+" and `exam_completed_archive`.`student_id` = "+student_id+" and `exam_completed_archive`.`chapter_id` != 0 and exam_completed_archive.subject_group_id = "+group_subject_id+" GROUP by exam_completed_archive.subject_id order by chapters.chapter_no asc")
		.then(async result=>{
			if(result.length > 0)
			{
				let subjectary = [];
...
let set_no = 0;
	setTimeout(async () => {
		let query_data = "";
		if(exam_type == 1){
		query_data = "select exam_completed_archive.*,subjects.name as subject_name,chapters.chapter_no,online_exam_question_answers.post_ans_status,online_exam_question_answers.post_ans,online_exam_question_answers.question_id,questions.question_type from `exam_completed_archive` left join `online_exam_question_answers` on `exam_completed_archive`.`exam_unique_id` = `online_exam_question_answers`.`exam_unique_id` left join subjects on subjects.id = exam_completed_archive.subject_id left join chapters on chapters.id = exam_completed_archive.chapter_id left join questions on questions.id = online_exam_question_answers.question_id where exam_completed_archive.previous_class = "+class_no+" and `exam_completed_archive`.`exam_type` = "+exam_type+" and `exam_completed_archive`.`student_id` = "+student_id+" and `exam_completed_archive`.`chapter_id` != 0 and exam_completed_archive.subject_group_id = "+group_subject_id+" order by chapters.chapter_no+0 asc";
		}
		else
		{
			query_data = "select exam_completed_archive.*,subjects.name as subject_name,chapters.chapter_no,online_exam_question_answers.post_ans_status,online_exam_question_answers.post_ans,online_exam_question_answers.question_id,questions.question_type from `exam_completed_archive` left join `online_exam_question_answers` on `exam_completed_archive`.`exam_unique_id` = `online_exam_question_answers`.`exam_unique_id` left join subjects on subjects.id = exam_completed_archive.subject_id left join chapters on chapters.id = exam_completed_archive.chapter_id left join questions on questions.id = online_exam_question_answers.question_id where exam_completed_archive.previous_class = "+class_no+" and `exam_completed_archive`.`exam_type` = "+exam_type+" and `exam_completed_archive`.`student_id` = "+student_id+" and `exam_completed_archive`.`chapter_id` != 0 and exam_completed_archive.subject_group_id = "+group_subject_id+" order by exam_completed_archive.exam_set_counter asc";
		}
...
let module_total_marks_ary = [];
		...
		await db.query("select online_exam_question_answers.post_ans_status,online_exam_question_answers.post_ans,exam_completed_archive.subject_id,questions.branch,questions.branch_id,questions.question_type,questions.id from `exam_completed_archive` left join online_exam_question_answers on online_exam_question_answers.exam_unique_id = exam_completed_archive.exam_unique_id left join questions on questions.id = online_exam_question_answers.question_id where exam_completed_archive.previous_class = "+class_no+" and `exam_category_id` = 1 and exam_completed_archive.exam_type = 2 and exam_completed_archive.chapter_id != 0 and exam_completed_archive.student_id = "+student_id+" and exam_completed_archive.subject_group_id = "+group_subject_id+"")
		.then(result=>{
			result.forEach(element=>{	
				if(module_total_marks_subject_ary[element['subject_id']] == null){
					module_total_marks_subject_ary[element['subject_id']]=[];				
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////// MOCK SECTION //////////////////////////
		await db.query("select online_exam_question_answers.post_ans_status,online_exam_question_answers.post_ans,exam_completed_archive.subject_id,questions.branch,questions.branch_id,questions.question_type,questions.id from `exam_completed_archive` left join online_exam_question_answers on online_exam_question_answers.exam_unique_id = exam_completed_archive.exam_unique_id left join questions on questions.id = online_exam_question_answers.question_id where exam_completed_archive.previous_class = "+class_no+" and `exam_category_id` = 1 and exam_completed_archive.exam_type = 3 and exam_completed_archive.chapter_id != 0 and exam_completed_archive.student_id = "+student_id+" and exam_completed_archive.subject_group_id = "+group_subject_id+"")
		.then(result=>{
			result.forEach(element=>{	
				if(mock_total_marks_subject_ary[element['subject_id']] == null){
					mock_total_marks_subject_ary[element['subject_id']]=[];				
	...
	for (const key in details_chapter_exam_ary_interm) {
		...
		for (const key2 in details_chapter_exam_ary_interm[key])
		{
			...
			let current_subject_data = await db.query("select * from `subjects` where is_deleted = 0 and status = 1 and name = '"+key+"' and exam_category_id = 1 and board_id = "+board);
			...
			let total_set_no = 1;
				...
				let total_exam_marks_ary = performance_full_score_chapterwise[subject_id][chapter_id];
			//console.log("select * from `exam_completed_archive` where `student_id` = "+student_id+" and exam_type = 1 and subject_id = "+subject_id+" and chapter_id = "+chapter_id);
			/*let set_counter = await db.query("select * from `exam_completed_archive` where exam_completed_archive.previous_class = "+class_no+" and `student_id` = "+student_id+" and exam_type = 1 and subject_id = "+subject_id+" and chapter_id = "+chapter_id+" and exam_completed_archive.subject_group_id = "+group_subject_id+"");
				total_set_no = set_counter.length;
			let total_exam_marks = 20 * total_set_no;
			*/
			let total_exam_marks = total_exam_marks_ary.reduce((acc, value) => acc + value, 0);
			if(exam_type == 2){
...
router.post('/scholastic_getchapterwise_analysis',adminMiddleware.validateToken , async function(req,res,next){
	...
	{	
		...
		let query = "";
		if(exam_type == 1){
			query = "select exam_completed_archive.*,subjects.name as subject_name,chapters.chapter_no,chapters.chapter_name,chapters.sub_heading as chapter_heading,chapters.order_no,online_exam_question_answers.post_ans_status,online_exam_question_answers.post_ans,online_exam_question_answers.question_id,questions.question_type from `exam_completed_archive` left join `online_exam_question_answers` on `exam_completed_archive`.`exam_unique_id` = `online_exam_question_answers`.`exam_unique_id` left join subjects on subjects.id = exam_completed_archive.branch_id left join chapters on chapters.id = exam_completed_archive.chapter_id left join questions on questions.id = online_exam_question_answers.question_id where exam_completed_archive.previous_class = "+class_no+" and `exam_completed_archive`.`exam_type` = "+exam_type+" and `exam_completed_archive`.`subject_id` = "+subject+" and `exam_completed_archive`.`chapter_id` = "+chapter+" and `exam_completed_archive`.`student_id` = "+student_id+" and `exam_completed_archive`.`subject_group_id` = "+group_subject_id+" and questions.question_type !='CSS'";

		}else if(exam_type > 1){
			query = "select exam_completed_archive.*,subjects.name as subject_name,chapters.chapter_no,chapters.chapter_name,chapters.sub_heading as chapter_heading,chapters.order_no,online_exam_question_answers.post_ans_status,online_exam_question_answers.post_ans,online_exam_question_answers.question_id,questions.question_type from `exam_completed_archive` left join `online_exam_question_answers` on `exam_completed_archive`.`exam_unique_id` = `online_exam_question_answers`.`exam_unique_id` left join subjects on subjects.id = exam_completed_archive.subject_id left join chapters on chapters.id = exam_completed_archive.chapter_id left join questions on questions.id = online_exam_question_answers.question_id where exam_completed_archive.previous_class = "+class_no+" and `exam_completed_archive`.`exam_type` = "+exam_type+" and `exam_completed_archive`.`subject_id` = "+subject+" and `exam_completed_archive`.`chapter_id` = "+chapter+" and `exam_completed_archive`.`student_id` = "+student_id+" and `exam_completed_archive`.`subject_group_id` = "+group_subject_id;
		}
...
if(exam_type != undefined && req.body.student_id != undefined)
{
		...
		let query_data_value = "select exam_completed_competitive_archive.*,online_exam_question_answers_competitive.question_id,online_exam_question_answers_competitive.post_ans,online_exam_question_answers_competitive.post_ans_status from `exam_completed_competitive_archive` left join online_exam_question_answers_competitive on \
		online_exam_question_answers_competitive.exam_unique_id = exam_completed_competitive_archive.exam_unique_id where exam_completed_competitive_archive.previous_class = "+class_no+" and `exam_completed_competitive_archive`.`exam_type` = '"+exam_type+"' and\
		 `exam_completed_competitive_archive`.`student_id` = "+req.body.student_id;
		...
		let query_data_value_market_trend = "select exam_completed_competitive_archive.* from `exam_completed_competitive_archive` where exam_completed_competitive_archive.previous_class = "+class_no+" and `exam_completed_competitive_archive`.`exam_type` = '"+exam_type+"' and `exam_completed_competitive_archive`.`student_id` != "+req.body.student_id;
...
{
	query_data = "select exam_completed_competitive_archive.*,online_exam_question_answers_competitive.question_id,online_exam_question_answers_competitive.post_ans,online_exam_question_answers_competitive.post_ans_status,questions.branch from `exam_completed_competitive_archive` left join online_exam_question_answers_competitive on online_exam_question_answers_competitive.exam_unique_id = exam_completed_competitive_archive.exam_unique_id join questions on questions.id = online_exam_question_answers_competitive.question_id where exam_completed_competitive_archive.previous_class = "+class_no+" and exam_completed_competitive_archive.exam_type = '"+exam_type+"' and online_exam_question_answers_competitive.student_id = "+user_id+" GROUP by online_exam_question_answers_competitive.question_id";
}else{
	query_data = "select exam_completed_competitive_archive.*,online_exam_question_answers_competitive.question_id,online_exam_question_answers_competitive.post_ans,online_exam_question_answers_competitive.post_ans_status,questions.branch from `exam_completed_competitive_archive` left join online_exam_question_answers_competitive on online_exam_question_answers_competitive.exam_unique_id = exam_completed_competitive_archive.exam_unique_id join questions on questions.id = online_exam_question_answers_competitive.question_id where exam_completed_competitive_archive.previous_class = "+class_no+" and online_exam_question_answers_competitive.student_id = "+user_id+" GROUP by online_exam_question_answers_competitive.question_id";
}
		await db.query(query_data)
		.then(result=>{
		...
		if(allow_exams_for_market_trend_ary.length > 0){
			...
			query_market_data = "select exam_completed_competitive_archive.*,online_exam_question_answers_competitive.question_id,online_exam_question_answers_competitive.post_ans,online_exam_question_answers_competitive.post_ans_status,questions.branch from `exam_completed_competitive_archive` left join online_exam_question_answers_competitive on \
		online_exam_question_answers_competitive.exam_unique_id = exam_completed_competitive_archive.exam_unique_id left join questions on questions.id = online_exam_question_answers_competitive.question_id where exam_completed_competitive_archive.previous_class = "+class_no+" and questions.is_deleted = 0 and questions.is_approve = 1 and questions.demo_exam = 0 and questions.exam_category = 2 and exam_completed_competitive_archive.exam_type = '"+exam_type+"' and exam_completed_competitive_archive.exam_unique_id in ("+allow_exams_for_market_trend_list+")";
		}
		else{
			query_market_data = "select exam_completed_competitive_archive.*,online_exam_question_answers_competitive.question_id,online_exam_question_answers_competitive.post_ans,online_exam_question_answers_competitive.post_ans_status,questions.branch from `exam_completed_competitive_archive` left join online_exam_question_answers_competitive on \
		online_exam_question_answers_competitive.exam_unique_id = exam_completed_competitive_archive.exam_unique_id left join questions on questions.id = online_exam_question_answers_competitive.question_id where exam_completed_competitive_archive.previous_class = "+class_no+" and questions.is_deleted = 0 and questions.is_approve = 1 and questions.demo_exam = 0 and questions.exam_category = 2 and exam_completed_competitive_archive.exam_type = '"+exam_type+"' and exam_completed_competitive_archive.exam_unique_id in ('')";
...
router.post('/getcompetitive_setwise_sat_mat_score',adminMiddleware.validateToken , async function(req,res,next){
    try{
		...
		await db.query("select questions.branch,questions.question_type, exam_completed_competitive_archive.*,online_exam_question_answers_competitive.question_id,online_exam_question_answers_competitive.post_ans,\
		online_exam_question_answers_competitive.post_ans_status from `exam_completed_competitive_archive` left join online_exam_question_answers_competitive on \
		online_exam_question_answers_competitive.exam_unique_id = exam_completed_competitive_archive.exam_unique_id left join questions on questions.id = online_exam_question_answers_competitive.question_id \
		where exam_completed_competitive_archive.previous_class = "+class_no+" and `exam_completed_competitive_archive`.`exam_type` = '"+exam_type+"' and `exam_completed_competitive_archive`.`exam_subtype_id` = "+exam_subtype_id+" and \
		`exam_completed_competitive_archive`.`exam_set_counter` = "+exam_set_counter+" and `exam_completed_competitive_archive`.`student_id` = "+req.body.student_id)
		.then(result=>{
			result.forEach(Element=>{
...
router.post('/getcompetitive_setwise_score',adminMiddleware.validateToken , async function(req,res,next){
    try{
		...
		if(exam_type != undefined && req.body.student_id != undefined)
		{
		let query_data = "select exam_completed_competitive_archive.*,online_exam_question_answers_competitive.question_id,online_exam_question_answers_competitive.post_ans,online_exam_question_answers_competitive.post_ans_status,questions.branch from `exam_completed_competitive_archive` left join online_exam_question_answers_competitive on \
		online_exam_question_answers_competitive.exam_unique_id = exam_completed_competitive_archive.exam_unique_id join questions on questions.id = online_exam_question_answers_competitive.question_id where exam_completed_competitive_archive.previous_class = "+class_no+" and `exam_completed_competitive_archive`.`student_id` = "+req.body.student_id+" and `exam_completed_competitive_archive`.`exam_type` = '"+exam_type+"'";
...
router.post('/getcompetitive_subjectwisecomparison',adminMiddleware.validateToken , async function(req,res,next){
    try{
		...
		await db.query("select exam_completed_competitive_archive.*,online_exam_question_answers_competitive.question_id,online_exam_question_answers_competitive.post_ans,online_exam_question_answers_competitive.post_ans_status,questions.branch from `exam_completed_competitive_archive` left join online_exam_question_answers_competitive on \
		online_exam_question_answers_competitive.exam_unique_id = exam_completed_competitive_archive.exam_unique_id left join questions on questions.id = online_exam_question_answers_competitive.question_id where exam_completed_competitive_archive.previous_class = "+class_no+" and `exam_completed_competitive_archive`.`student_id` = "+req.body.student_id+" and exam_completed_competitive_archive.exam_type = '"+exam_type+"' and exam_completed_competitive_archive.exam_set_counter ="+set_no+" GROUP by online_exam_question_answers_competitive.question_id")
		.then(result=>{
...
router.post('/getcompetitive_nonverbalcomparison',adminMiddleware.validateToken , async function(req,res,next){
	...
	let allow_setno_for_calculation = 0;
		...
		await db.query("select exam_completed_competitive_archive.*,online_exam_question_answers_competitive.question_id,online_exam_question_answers_competitive.post_ans,online_exam_question_answers_competitive.post_ans_status,questions.branch from `exam_completed_competitive_archive` left join online_exam_question_answers_competitive on \
		online_exam_question_answers_competitive.exam_unique_id = exam_completed_competitive_archive.exam_unique_id join questions on questions.id = online_exam_question_answers_competitive.question_id where exam_completed_competitive_archive.previous_class = "+class_no+" and exam_completed_competitive_archive.exam_type = '"+exam_type+"' and exam_completed_competitive_archive.student_id = "+student_id+" and questions.branch = '"+subject_code+"' GROUP by online_exam_question_answers_competitive.question_id")
		.then(result=>{
			result.forEach(Element=>{
				if(allow_setno_for_calculation >= Element['exam_set_counter'])
				{
...
router.post('/getcompare_diffarent_subject_score_ntse',adminMiddleware.validateToken , async function(req,res,next){
    try{
		...
		let subject_ary = [];
		let total_record_ary = {};
		await db.query("select exam_completed_competitive_archive.*,online_exam_question_answers_competitive.question_id,\
		online_exam_question_answers_competitive.post_ans,online_exam_question_answers_competitive.post_ans_status,\
		questions.branch,subjects.name as subject_name,subjects.id as subject_id from `exam_completed_competitive_archive` left join online_exam_question_answers_competitive on \
		online_exam_question_answers_competitive.exam_unique_id = exam_completed_competitive_archive.exam_unique_id \
		...
		where exam_completed_competitive_archive.previous_class = "+class_no+" and `exam_completed_competitive_archive`.`exam_type` = 'NTSE' and `exam_completed_competitive_archive`.`student_id` = "+req.body.student_id)
		.then(result=>{
			result.forEach(Element=>{
...
router.post('/getcompetitive_specific_setwise_score_ntse' ,adminMiddleware.validateToken , async function(req,res,next){
    try{
		...
		await db.query("select exam_completed_competitive_archive.*,online_exam_question_answers_competitive.question_id,online_exam_question_answers_competitive.post_ans,online_exam_question_answers_competitive.post_ans_status,questions.branch from `exam_completed_competitive_archive` left join online_exam_question_answers_competitive on \
		online_exam_question_answers_competitive.exam_unique_id = exam_completed_competitive_archive.exam_unique_id join questions on questions.id = online_exam_question_answers_competitive.question_id where exam_completed_competitive_archive.previous_class = "+class_no+" and \
		`exam_completed_competitive_archive`.`exam_set_counter` = "+req.body.set_no+" and `exam_completed_competitive_archive`.`student_id` = "+req.body.student_id)
		.then(result=>{
			result.forEach(Element=>{
				if(total_correct_ans_ary[Element['exam_set_counter']] == null)
				{
...
router.post('/compare_scholastic_competitive' ,adminMiddleware.validateToken , async function(req,res,next){
	...
	if(exam_type != undefined)
	{
	await db.query("select subjects.* from `subjects` left join  exam_type on  exam_type.id = subjects.exam_type_id where `subjects`.`exam_category_id` = 1 and `subjects`.`is_deleted` = 0 and `subjects`.`status` = 1 and `subjects`.`group_exist` = 2")
	.then(result=>{
...
if(exam_type !='Competitive')
{
	query_data = "select subjects.* from `subjects` left join  exam_type on  exam_type.id = subjects.exam_type_id where `subjects`.`exam_category_id` = 2 and `subjects`.`is_deleted` = 0 and `subjects`.`status` = 1 and `subjects`.`group_exist` = 2 and  exam_type.type_name = '"+exam_type+"' order by name ASC";
}else{
	query_data = "select subjects.* from `subjects` left join  exam_type on  exam_type.id = subjects.exam_type_id where `subjects`.`exam_category_id` = 2 and `subjects`.`is_deleted` = 0 and `subjects`.`status` = 1 and `subjects`.`group_exist` = 2 order by name ASC";
}
	await db.query(query_data)
	.then(result=>{
		if(result_data['labels'] == null){
	...
	await db.query("select online_exam_question_answers.*,questions.branch,questions.question_type,question_pattern.marks from `online_exam_question_answers` left join exam_completed_archive on exam_completed_archive.exam_unique_id = online_exam_question_answers.exam_unique_id left join questions on online_exam_question_answers.question_id = questions.id left JOIN question_pattern on question_pattern.short_code = questions.question_type where exam_completed_archive.previous_class = "+class_no+" and `online_exam_question_answers`.`student_id` = "+user_id+" and exam_completed_archive.subject_group_id = "+group_subject_id)
	.then(result=>{
		result.forEach(element=>{
...
if(exam_type !='Competitive')
{	
		query_data_search = "select exam_completed_competitive_archive.*,online_exam_question_answers_competitive.question_id,online_exam_question_answers_competitive.post_ans,online_exam_question_answers_competitive.post_ans_status from `exam_completed_competitive_archive` left join online_exam_question_answers_competitive on \
		online_exam_question_answers_competitive.exam_unique_id = exam_completed_competitive_archive.exam_unique_id where exam_completed_competitive_archive.previous_class = "+class_no+" and `exam_completed_competitive_archive`.`exam_type` = '"+exam_type+"' and\
		 `exam_completed_competitive_archive`.`student_id` = "+req.body.student_id;
}else{
	query_data_search = "select exam_completed_competitive_archive.*,online_exam_question_answers_competitive.question_id,online_exam_question_answers_competitive.post_ans,online_exam_question_answers_competitive.post_ans_status from `exam_completed_competitive_archive` left join online_exam_question_answers_competitive on \
		online_exam_question_answers_competitive.exam_unique_id = exam_completed_competitive_archive.exam_unique_id where exam_completed_competitive_archive.previous_class = "+class_no+" and `exam_completed_competitive_archive`.`student_id` = "+req.body.student_id;
}
...
if(exam_type !='Competitive')
{
	...
	await db.query("select online_exam_question_answers.*,questions.branch,questions.question_type,question_pattern.marks,exam_completed_archive.subject_group_id as group_subject_id from `online_exam_question_answers` left join exam_completed_archive on exam_completed_archive.exam_unique_id = online_exam_question_answers.exam_unique_id left join questions on online_exam_question_answers.question_id = questions.id left JOIN question_pattern on question_pattern.short_code = questions.question_type where exam_completed_archive.previous_class = "+class_no+" and `online_exam_question_answers`.`student_id` = "+user_id)
	.then(result=>{
		result.forEach(element=>{
...
router.post('/elibrary_session_time' ,adminMiddleware.validateToken , async function(req,res,next){
	...
	try{
		...
		await db.query("SELECT sum(elibrary_access_log_archive.time_spend) as total_time,elibrary_access_log_archive.subject_id,subjects.name as subject_name,subjects.exam_category_id,exam_categories.category FROM `elibrary_access_log_archive` left join subjects on subjects.id = elibrary_access_log_archive.subject_id left join exam_categories on exam_categories.id = subjects.exam_category_id WHERE elibrary_access_log_archive.previous_class = "+class_no+" and elibrary_access_log_archive.student_id ="+user_id+" and subject_id in ("+data+") group by subjects.name")
		.then(result=>{
			result.forEach(element=>{
				subject_ary.push(element.subject_id);
...
router.post('/elibrary_most_visited_subjects' ,adminMiddleware.validateToken , async function(req,res,next){
	...
	Promise1.then(async data=>{
		await db.query("SELECT count(elibrary_visit_log_archive.subject_id) as most_visit,elibrary_visit_log_archive.subject_id,subjects.name as subject_name,subjects.exam_category_id,exam_categories.category FROM `elibrary_visit_log_archive` left join subjects on subjects.id = elibrary_visit_log_archive.subject_id left join exam_categories on exam_categories.id = subjects.exam_category_id WHERE elibrary_visit_log_archive.previous_class = "+class_no+" and elibrary_visit_log_archive.student_id ="+user_id+" and elibrary_visit_log_archive.subject_id IN ("+data+") group by subjects.name order by most_visit desc")
		.then(result=>{
			result.forEach(element=>{
				subject_ary.push(element.subject_id);
				categories.push(element.subject_name);
...
router.post('/elibrary_most_search_questions' ,adminMiddleware.validateToken , async function(req,res,next){
	...
	Promise1.then(async data=>{
		await db.query("SELECT count(subject_id) as total_search,searched_questions_archive.search_text,subjects.name as subject_name from `searched_questions_archive` left join subjects on searched_questions_archive.subject_id = subjects.id where `searched_questions_archive`.`previous_class` = "+class_no+" and `searched_questions_archive`.`student_id` = "+user_id+" and `searched_questions_archive`.`subject_id` IN ("+data+") group by subjects.name order by total_search desc limit 10")
		.then(result=>{
			result.forEach(element=>{
				subject_ary.push(element.subject_id);
				categories.push(element.subject_name);
...
router.post('/wheredoyoustand_competitive',adminMiddleware.validateToken , async function(req,res,next){
    try{
		...
		await db.query("select exam_completed_competitive_archive.*,questions.question_type,online_exam_question_answers_competitive.question_id,online_exam_question_answers_competitive.post_ans,online_exam_question_answers_competitive.post_ans_status,questions.branch,questions.chapter_id,questions.chapter,subjects.name as subject_name,subjects.subject_code,chapters.sub_heading as chapter_name from `exam_completed_competitive_archive` left join online_exam_question_answers_competitive on online_exam_question_answers_competitive.exam_unique_id = exam_completed_competitive_archive.exam_unique_id join questions on questions.id = online_exam_question_answers_competitive.question_id left join subjects on subjects.subject_code = questions.branch left join chapters on chapters.id = questions.chapter_id where exam_completed_competitive_archive.previous_class = "+class_no+" and `questions`.`exam_category`= 2 and `exam_completed_competitive_archive`.`student_id` = "+user_id+" and `exam_completed_competitive_archive`.`exam_type` = '"+exam_type+"' GROUP by `online_exam_question_answers_competitive`.`question_id`")
		.then(result=>{
			/*if(result.length > 0){
			result.forEach(Element=>{
		...
		if(finaldataary[element]!=undefined){	
...
router.post('/scholastic_getsubjectwise_chapters_table_data',adminMiddleware.validateToken , async function(req,res,next){
	...
	{
		await db.query("select exam_completed_archive.*,subjects.name as subject_name from `exam_completed_archive` left join subjects on subjects.id = exam_completed_archive.subject_id where exam_completed_archive.previous_class = "+class_no+" and `exam_completed_archive`.`exam_type` = "+exam_type+" and `subjects`.`name` = '"+subject_name+"' and `exam_completed_archive`.`student_id` = "+student_id+" and `exam_completed_archive`.`subject_group_id` = "+group_subject_id)
		.then(result=>{
		...
		let query = "";
		if(exam_type == 1){
			query = "select exam_completed_archive.*,subjects.name as subject_name,chapters.chapter_no,chapters.chapter_name,chapters.sub_heading as chapter_heading,chapters.order_no,online_exam_question_answers.post_ans_status,online_exam_question_answers.post_ans,online_exam_question_answers.question_id,questions.question_type from `exam_completed_archive` left join `online_exam_question_answers` on `exam_completed_archive`.`exam_unique_id` = `online_exam_question_answers`.`exam_unique_id` left join subjects on subjects.id = exam_completed_archive.branch_id left join chapters on chapters.id = exam_completed_archive.chapter_id left join questions on questions.id = online_exam_question_answers.question_id where exam_completed_archive.previous_class = "+class_no+" and `exam_completed_archive`.`exam_type` = "+exam_type+" and `exam_completed_archive`.`subject_id` = "+subject+" and `exam_completed_archive`.`student_id` = "+student_id+" and `exam_completed_archive`.`subject_group_id` = "+group_subject_id;
