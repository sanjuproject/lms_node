...
if(req.body.group_subject_id == undefined || req.body.group_subject_id == "" || req.body.group_subject_id == 0){
	...
	.then(async result=>{
		...
		"question_type":questionsdetails[element.question_no],"subject_id":subject_id,"group_subject_id":group_subject_id,user_data:userdata,"case_study_exam":case_study_exam});
			...
			.then(async(result)=>{
				...
				if(exam_type == 1){
					...
					{	
						...
						await db.query("update `exam_completed` set `created_at` = '"+isoDateTime+"' where `exam_unique_id` = '"+exam_unique_id+"'")
						.then(async result=>{
			...
			.then(async (result)=>{
				...
				.then(async result=>{
					...
					await db.query("delete from `interm_storeexamdata` where `student_id` = "+student_id+" and `exam_type` = "+exam_type_id+" and `set_no` = "+exam_set_counter+" and `branch` = "+exam_subtype)
						...
						await db.query("update `exam_completed_competitive` set `created_at` = '"+isoDateTime+"' where `exam_unique_id` = '"+exam_unique_id+"'")
						.then(async result=>{
